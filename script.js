...
// Version: 5.00.2025
// Author: Institutional Developer
// Description: Full-featured trading platform with multi-asset support,
// advanced charting, technical indicators, drawing tools, order management,
// risk engine, backtesting, and psychology simulation.
// ============================================================================
...
function loadUserPreferences() {
    const saved = localStorage.getItem('mt5_settings');
    if (saved) {
        try {
            const prefs = JSON.parse(saved);
            Object.assign(AppState.settings, prefs);
        } catch (e) {}
    }
}
...
function initializeMarketData() {
    // Create symbols from categories
    AppState.symbols = [];
    for (const category in SYMBOL_CATEGORIES) {
        SYMBOL_CATEGORIES[category].forEach(sym => {
            AppState.symbols.push({
                ...
                volume: 0,
                session: '24/5',
                leverage: category === 'CRYPTO' ? 50 : (category === 'FOREX' ? 500 : 20)
            });
        });
    }
...
function generateChartData(symbol, bars) {
    ...
    for (let i = 0; i < bars; i++) {
        ...
        data.push({
            time: new Date(Date.now() - (bars - i) * 60000 * TIMEFRAMES[AppState.activeTimeframe]),
            open: open,
            high: high,
            low: low,
            close: close,
...
function drawLine(values, color, width) {
    // Implementation draws a line on chart
...
function drawCrosshair() {
    // Crosshair drawing (to be implemented with mouse events)
}
...
function drawPriceScale() {
    ...
    for (let i = 0; i <= 5; i++) {
        ...
        const y = chartCanvas.height - 40 - (i * (chartCanvas.height - 40) / 5);
